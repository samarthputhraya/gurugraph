...
def _context(topic, analysis, students, critiques=None):
    return {
        "analysis": analysis,
        "concept_names": {c: topic.concept_name(c) for c in topic.order},
        "worked_example": _worked_example(topic, analysis["focus_concept"]),
        "next_steps_seed": _next_steps_seed(topic, students),
        "critiques": critiques,
    }
...
def _template(topic: Topic, analysis: dict, students: dict[str, StudentRecord]) -> dict:
    ...
    return {
        "recommendations": [
            {
                "group_label": f"Group A, {len(group)} students",
                "concept_id": focus,
...
def propose(topic: Topic, analysis: dict, students: dict[str, StudentRecord], llm: LLM) -> tuple[dict, str]:
    ...
    try:
        out = llm.generate(
            system=prompts.COACH_PROPOSE,
            prompt=prompt,
            schema=schemas.CoachProposal,
...
def revise(
    topic: Topic, analysis: dict, students: dict[str, StudentRecord], proposal: dict, critiques: list[dict], llm: LLM
) -> tuple[dict, str]:
    prompt = (
        f"Class analysis:\n{json.dumps(analysis)}\n\nYour proposals:\n{json.dumps(proposal)}\n\n"
...
def parent_template(ctx: dict) -> dict:
    ...
    return {
        "language": "en",
        "message": (
            f"Today {ctx['nickname']} practised {ctx['concept_name'].lower()} and kept trying on the hard "
