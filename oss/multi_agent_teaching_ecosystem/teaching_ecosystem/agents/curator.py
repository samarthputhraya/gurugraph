...
Lessons are cached per (topic, concept, misconception, language), which is also how the production app keeps
...
def _practice_seed(topic: Topic, concept_id: str) -> list[dict]:
    ...
    mcq = [
        {"stem": q["stem"], "answer": next(o["text"] for o in q["options"] if o.get("correct"))}
...
def lesson(
    topic: Topic, concept_id: str, tag: str, language: str, llm: LLM, cache: dict | None = None
) -> tuple[dict, str]:
    key = f"{topic.id}|{concept_id}|{tag}|{language}"  # topic.id keeps a shared cache safe across topic packs
    ...
    ctx = {
        "language": language,
        "definition": topic.definition(tag),
        "method": method,
